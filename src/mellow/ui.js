...
export function Input(data) {
  if(data[0].function == null) {
     ...
     var functs = data.function;
  }
  if(data[0].return == null) {
     var rets = "";
  } else {
     var rets = `x-model="${data.return}"`
  }
  return `<label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">${data.labelText}</label>
        <input type="${data.type}" id="${data.id}" {$rets} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="${data.placeHolder}" ${functs}>`
}